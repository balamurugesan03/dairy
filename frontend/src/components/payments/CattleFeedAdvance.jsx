...
const CattleFeedAdvance = () => {
  ...
  /* ── Fetch Register (all producers, all transactions) ── */
  const fetchRegister = async () => {
    setLoading(true);
    try {
      const res = await cattleFeedAdvanceAPI.getRegister({
        fromDate: dayjs(fromDate).format('YYYY-MM-DD'),
        toDate:   dayjs(toDate).format('YYYY-MM-DD'),
      });
      if (!res.success) throw new Error(res.message || 'Failed');
  ...
  const exportRegister = () => {
    ...
    if (registerTotals) {
      data.push({
        ...
        'Ref No': '', 'Transaction Type': '', 'Description': '', 'Item': '',
        'Debit (Recovery)': registerTotals.debit,
        'Credit (Sales)':   registerTotals.credit,
      });
    }
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data), 'CF Register');
    XLSX.writeFile(wb, `CF_Register_${dayjs(fromDate).format('MMYYYY')}.xlsx`);
  };
      ...
      <Paper withBorder shadow="sm" radius="md" style={{ position: 'relative' }}>
        ...
        <Tabs value={activeTab} onChange={setActiveTab}>
          <Tabs.List px="sm" pt="xs" className="no-print">
            ...
            <Tabs.Tab value="register" fw={600}>CF Advance Register</Tabs.Tab>
            <Tabs.Tab value="ledger"   fw={600}>Producer Ledger</Tabs.Tab>
          </Tabs.List>

          {/* ── SUMMARY TAB ── */}
          <Tabs.Panel value="summary" p="md">
            <div id="cfa-print" ref={activeTab === 'summary' ? printRef : undefined}>
              {/* Print header */}
          ...
          <Tabs.Panel value="register" p="md">
            <div id="cfa-print" ref={activeTab === 'register' ? printRef : undefined}>
              {/* Print header */}
              <Box ta="center" mb="sm" style={{ display: 'none' }} className="print-header">
                <Text fw={700} size="lg">{selectedCompany?.companyName || 'Dairy Cooperative Society'}</Text>
                <Text fw={600} size="md">CATTLE FEED ADVANCE — REGISTER</Text>
                <Text size="sm">
                  Period: {dayjs(fromDate).format('DD/MM/YYYY')} to {dayjs(toDate).format('DD/MM/YYYY')}
                </Text>
                <Divider my="xs" />
                ...
                <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 1050 }}>
                  ...
                  <tbody>
                    {registerRows.length === 0 ? (
                      <tr>
                        <td colSpan={10} style={{ ...TD({ textAlign: 'center', color: '#999', padding: '24px' }) }}>
                          Click Generate to load data
                        </td>
                      </tr>
                    ) : (
                    ...
                    {registerTotals && registerRows.length > 0 && (
                      <tr style={{ borderTop: '2px solid #0d3b6e', background: '#edf2f7' }}>
                        <td colSpan={8} style={{ ...TD({ fontWeight: 700, textAlign: 'center', background: '#2d3748', color: '#fff', letterSpacing: 1 }) }}>
                          TOTAL
                        </td>
