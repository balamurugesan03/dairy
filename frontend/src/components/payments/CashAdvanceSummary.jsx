...
const PRINT_STYLE = `
  @media print {
    ...
    #cash-print, #cash-print * { visibility: visible !important; }
    #cash-print { position: fixed; inset: 0; padding: 12px; }
...
const TYPE_LABEL = {
  ADVANCE:  { label: 'Cash Advance',   color: '#7b341e' },
  RECOVERY: { label: 'Cash Recovery',  color: '#276749' },
};
...
const CashAdvanceSummary = () => {
  ...
  const fetchSummary = async () => {
    setLoading(true);
    try {
      const res = await cashAdvanceAPI.getSummary({
  ...
  const fetchLedger = async () => {
    ...
    setLoading(true);
    try {
      const res = await cashAdvanceAPI.getLedger({
        farmerId,
        fromDate: dayjs(fromDate).format('YYYY-MM-DD'),
        toDate:   dayjs(toDate).format('YYYY-MM-DD'),
      });
  ...
  const fetchRegister = async () => {
    setLoading(true);
    try {
      const res = await cashAdvanceAPI.getRegister({
        fromDate: dayjs(fromDate).format('YYYY-MM-DD'),
        toDate:   dayjs(toDate).format('YYYY-MM-DD'),
      });
      if (!res.success) throw new Error(res.message || 'Failed');
  ...
  const exportRegister = () => {
    ...
    if (registerTotals) {
      data.push({
        ...
        'Credit (Advance)': registerTotals.credit,
      });
    }
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data), 'Cash Register');
    XLSX.writeFile(wb, `Cash_Register_${dayjs(fromDate).format('MMYYYY')}.xlsx`);
  };
  ...
  const exportLedger = () => {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(data), 'Cash Ledger');
    XLSX.writeFile(wb, `Cash_Ledger_${farmerLabel.replace(/[^a-z0-9]/gi, '_')}_${dayjs(fromDate).format('MMYYYY')}.xlsx`);
  };
        ...
        <Group justify="space-between" align="flex-end" wrap="wrap" gap="sm">
          <Box>
            ...
            <Text c="dimmed" size="sm">
              Producer-wise cash advance ledger — disbursements, recoveries &amp; running outstanding
            </Text>
          </Box>
        </Group>
      </Box>
      ...
      <Paper withBorder shadow="sm" radius="md" style={{ position: 'relative' }}>
        ...
        <Tabs value={activeTab} onChange={setActiveTab}>
          <Tabs.List px="sm" pt="xs" className="no-print">
            <Tabs.Tab value="summary"  fw={600}>Outstanding Balance Report</Tabs.Tab>
            <Tabs.Tab value="register" fw={600}>Cash Advance Register</Tabs.Tab>
            <Tabs.Tab value="ledger"   fw={600}>Producer Ledger</Tabs.Tab>
          </Tabs.List>

          {/* ── SUMMARY TAB ── */}
          <Tabs.Panel value="summary" p="md">
            <div id="cash-print" ref={activeTab === 'summary' ? printRef : undefined}>
              <Box ta="center" mb="sm" style={{ display: 'none' }} className="print-header">
                ...
                <Text fw={600} size="md">CASH ADVANCE — SUMMARY</Text>
                <Text size="sm">
          ...
          <Tabs.Panel value="register" p="md">
            <div id="cash-print" ref={activeTab === 'register' ? printRef : undefined}>
              <Box ta="center" mb="sm" style={{ display: 'none' }} className="print-header">
                <Text fw={700} size="lg">{selectedCompany?.companyName || 'Dairy Cooperative Society'}</Text>
                <Text fw={600} size="md">CASH ADVANCE — REGISTER</Text>
                <Text size="sm">
                  Period: {dayjs(fromDate).format('DD/MM/YYYY')} to {dayjs(toDate).format('DD/MM/YYYY')}
                </Text>
                <Divider my="xs" />
                ...
                <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: 960 }}>
                  ...
                  <thead>
                    <tr>
                      ...
                      <th style={TH('#6e2c00')}>Debit (Recovery)</th>
                      <th style={TH('#155724')}>Credit (Advance)</th>
                    </tr>
                  </thead>
                  <tbody>
          ...
          <Tabs.Panel value="ledger" p="md">
            ...
            <div id="cash-print" ref={activeTab === 'ledger' ? printRef : undefined}>
              {ledgerData ? (
                <>
                  <Box ta="center" mb="sm" style={{ display: 'none' }} className="print-header">
                    <Text fw={700} size="lg">{selectedCompany?.companyName || 'Dairy Cooperative Society'}</Text>
                    <Text fw={600} size="md">CASH ADVANCE — PRODUCER LEDGER</Text>
