...
const SupplierModal = ({ isOpen, onClose, onSuccess, supplierId = null }) => {
  ...
  const fetchNextSupplierId = async () => {
    setLoadingId(true);
    try {
      const response = await supplierAPI.getNextId();
      setFormData(prev => ({ ...prev, supplierId: response.data.supplierId }));
    } catch (error) {
  ...
  const validateForm = () => {
    ...
    // Supplier ID is auto-generated, so no validation needed for create mode
    if (isEditMode && !formData.supplierId.trim()) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notifications.show({
        title: 'Error',
        message: error.message || `Failed to ${isEditMode ? 'update' : 'create'} supplier`,
        color: 'red'
      });
    } finally {
      setLoading(false);
    ...
    <Modal
      ...
      title={
        <Group>
          <IconUser size={24} />
          <div>
            <Title order={4}>{isEditMode ? 'Edit Supplier' : 'Add New Supplier'}</Title>
            <Text size="sm" c="dimmed">
              {isEditMode ? 'Update the details of the supplier' : 'Fill in the details to create a new supplier'}
            </Text>
          </div>
        </Group>
      }
      ...
      <Stack gap="md">
        ...
        <Grid>
          <Grid.Col span={{ base: 12, sm: 6 }}>
            <TextInput
              label="Supplier ID"
              placeholder={loadingId ? "Generating..." : "Auto-generated"}
              ...
              readOnly
              disabled={loadingId}
              description={!isEditMode ? "Auto-generated (SUP0001 format)" : undefined}
