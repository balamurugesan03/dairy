...
const PaymentVoucher = () => {
  ...
  const handlePrint = (voucher) => {
    ...
    setTimeout(() => {
      ...
      printWindow.document.write(`
        ...
        <html>
        <head>
          <title>Payment Voucher - ${voucher.voucherNumber || 'N/A'}</title>
          <style>
        ...
        <body>
          <div class="voucher-container">
            ...
            <div class="info-row bordered">
              ...
              <span class="value">${voucher.entries?.find(e => e.debitAmount > 0)?.ledgerName || '-'}</span>
            </div>
  ...
  const handleSaveMultiple = async () => {
    ...
    if (successCount > 0) {
      message.success(`${successCount} payment voucher(s) created successfully`);
      setModalOpened(false);
      fetchVouchers();
    }
    if (errorCount > 0) {
  ...
  const selectData = useMemo(() => {
    ...
    ledgers
      ...
      // 'R' (Receipt Side Only) must never be selectable as the head of
      // account in a Payment Voucher — only 'P' (Payment Side Only) and 'B'
      // (Both) ledgers qualify. Ledgers with no voucherType recorded yet
      // (legacy data) are left visible rather than hidden. See PaymentVoucher
      // fix notes: backend enforces the same rule in createVoucher.
      .filter(ledger => ledger.voucherType !== 'R')
      .forEach(ledger => {
    ...
    <Container size="xl" py="md">
        ...
        {vouchers.length === 0 ? (
          <Card p="xl" withBorder bg="gray.0">
            <Stack align="center" gap="md">
              ...
              <Text c="dimmed" size="lg">No payment vouchers found</Text>
              <Button
                variant="light"
                leftSection={<IconPlus size={16} />}
                onClick={openAddModal}
        ...
        {vouchers.length > 0 && (
          <Card mt="md" p="md" withBorder bg="red.0">
            <Group justify="space-between">
              <Text fw={500}>Total Payments</Text>
      ...
      <Modal
        ...
        title={
          <Group gap="xs">
            <IconCreditCard size={20} />
            <Text fw={600}>Add Payment Voucher</Text>
          </Group>
        }
          ...
          {entryMode === 'single' ? (
            ...
            <Stack gap="md">
              <Alert color="red" variant="light" py="xs">
                <Text size="xs">
                  Add multiple payment vouchers at once. Cash account will be credited automatically for each entry.
                </Text>
              </Alert>
      ...
      <Modal
        ...
        title={
          <Group gap="xs">
            <IconEye size={20} />
            <Text fw={600}>Payment Voucher Details</Text>
          </Group>
        }
        size="md"
        centered
          ...
          <Stack gap="md">
            <Grid>
              ...
              <Grid.Col span={12}>
                ...
                <Text fw={500}>{selectedVoucher.entries?.find(e => e.debitAmount > 0)?.ledgerName || '-'}</Text>
              </Grid.Col>
              <Grid.Col span={12}>
      ...
      <div id="print-payment" style={{ display: 'none' }} ref={printRef}></div>
    </Container>
  );
};
