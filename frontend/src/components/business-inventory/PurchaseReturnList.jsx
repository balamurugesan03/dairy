/**
 * Purchase Return List - Debit Note Management
 * ==============================================
 * View, search, filter and manage all purchase returns
 */
...
const PurchaseReturnList = () => {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const response = await purchaseReturnAPI.getAll(params);
  ...
  const fetchSummary = async () => {
    try {
      ...
      const response = await purchaseReturnAPI.getSummary(params);
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await purchaseReturnAPI.delete(selectedReturn._id);
      notifications.show({
        title: 'Success',
      ...
      notifications.show({
        title: 'Error',
        message: error.response?.data?.message || 'Failed to delete purchase return',
        color: 'red'
      });
    } finally {
      setDeleting(false);
    ...
    <Container size="xl" py="md">
      ...
      <Paper withBorder p="md" mb="md" radius="md">
        <Group justify="space-between" align="center">
          <Group>
            ...
            <div>
              <Title order={3}>Purchase Returns</Title>
              <Text size="sm" c="dimmed">Manage debit notes and purchase returns</Text>
            </div>
          </Group>
          <Button
            leftSection={<IconPlus size={16} />}
            onClick={() => navigate('/business-inventory/purchase-returns/new')}
            color="red"
          >
            New Purchase Return
          </Button>
      ...
      <Paper withBorder radius="md">
        <DataTable
          ...
          fetching={loading}
          minHeight={400}
          noRecordsText="No purchase returns found"
          highlightOnHover
          verticalSpacing="sm"
