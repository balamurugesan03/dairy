/**
 * Sales Return List - Credit Note Management
 * ============================================
 * View, search, filter and manage all sales returns
 */
...
const SalesReturnList = () => {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const response = await salesReturnAPI.getAll(params);
  ...
  const fetchSummary = async () => {
    try {
      ...
      const response = await salesReturnAPI.getSummary(params);
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await salesReturnAPI.delete(selectedReturn._id);
      notifications.show({
        title: 'Success',
      ...
      notifications.show({
        title: 'Error',
        message: error.response?.data?.message || 'Failed to delete sales return',
        color: 'red'
      });
    } finally {
      setDeleting(false);
        ...
        <Group gap={4} justify="flex-end">
          <Tooltip label="View">
            <ActionIcon
              variant="light"
              size="sm"
              onClick={() => navigate(`/business-inventory/sales-returns/${record._id}?type=sales`)}
            >
              <IconEye size={14} />
            </ActionIcon>
          </Tooltip>
          <Menu position="bottom-end" shadow="md">
            ...
            <Menu.Dropdown>
              <Menu.Item
                ...
                onClick={() => navigate(`/business-inventory/sales-returns/edit/${record._id}?type=sales`)}
              >
                Edit
              </Menu.Item>
              <Menu.Divider />
    ...
    <Container size="xl" py="md">
      ...
      <Paper withBorder p="md" mb="md" radius="md">
        <Group justify="space-between" align="center">
          <Group>
            ...
            <div>
              <Title order={3}>Sales Returns</Title>
              <Text size="sm" c="dimmed">Manage credit notes and sales returns</Text>
            </div>
          </Group>
          <Button
            leftSection={<IconPlus size={16} />}
            onClick={() => navigate('/business-inventory/sales-returns/new?type=sales')}
            color="orange"
          >
            New Sales Return
          </Button>
      ...
      <Paper withBorder radius="md">
        <DataTable
          ...
          fetching={loading}
          minHeight={400}
          noRecordsText="No sales returns found"
          highlightOnHover
          verticalSpacing="sm"
