/**
 * Dairy Sales Return List - Credit Note Management
 * ==================================================
 * View, search, filter and manage all dairy sales returns
 */
...
const DairySalesReturnList = () => {
  ...
  const fetchReturns = async () => {
    try {
      ...
      const response = await dairySalesReturnAPI.getAll(params);
      ...
      notifications.show({
        title: 'Error',
        message: 'Failed to fetch dairy sales returns',
        color: 'red'
      });
      setReturns([]);
    } finally {
  ...
  const fetchSummary = async () => {
    try {
      ...
      const response = await dairySalesReturnAPI.getSummary(params);
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await dairySalesReturnAPI.delete(selectedReturn._id);
      notifications.show({
        title: 'Success',
        message: 'Dairy sales return deleted successfully',
        color: 'green'
      });
      ...
      notifications.show({
        title: 'Error',
        message: error.response?.data?.message || 'Failed to delete dairy sales return',
        color: 'red'
      });
    } finally {
      setDeleting(false);
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (record) => (
        <Box>
          <Text size="sm" fw={500}>{record.customerName || '-'}</Text>
          {record.customerPhone && (
            <Text size="xs" c="dimmed">{record.customerPhone}</Text>
          )}
          {record.customerType && (
            <Badge size="xs" variant="light" color="gray">{record.customerType}</Badge>
          )}
        </Box>
      )
    },
        ...
        <Group gap={4} justify="flex-end">
          <Tooltip label="View">
            <ActionIcon
              variant="light"
              size="sm"
              onClick={() => navigate(`/inventory/sales-returns/${record._id}?type=sales`)}
            >
              <IconEye size={14} />
            </ActionIcon>
          </Tooltip>
          <Menu position="bottom-end" shadow="md">
            ...
            <Menu.Dropdown>
              <Menu.Item
                ...
                onClick={() => navigate(`/inventory/sales-returns/edit/${record._id}?type=sales`)}
              >
                Edit
              </Menu.Item>
              <Menu.Divider />
    ...
    <Container size="xl" py="md">
      ...
      <Paper withBorder p="md" mb="md" radius="md">
        <Group justify="space-between" align="center">
          <Group>
            ...
            <div>
              <Title order={3}>Dairy Sales Returns</Title>
          ...
          <Card withBorder p="md" radius="md">
            <Group justify="space-between">
              <div>
                ...
                <Text size="xl" fw={700} c="green">{(summary.totalPaid || 0).toFixed(2)}</Text>
              </div>
      ...
      <Paper withBorder radius="md">
        <DataTable
          ...
          fetching={loading}
          minHeight={400}
          noRecordsText="No dairy sales returns found"
          highlightOnHover
          verticalSpacing="sm"
      ...
      <Modal
        ...
        title="Delete Dairy Sales Return"
        centered
      >
        <Stack>
          <Alert color="red" icon={<IconAlertCircle size={16} />}>
            <Text size="sm">
              Are you sure you want to delete credit note <strong>{selectedReturn?.returnNumber}</strong>?
...
export default DairySalesReturnList;
