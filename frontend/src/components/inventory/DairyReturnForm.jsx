/**
 * Dairy Return Form - Purchase Return (Debit Note) & Sales Return (Credit Note)
 * ==============================================================================
 ...
 * Uses Item + StockTransaction (Dairy inventory)
 */
...
const DairyReturnForm = () => {
  ...
  // Derive from URL pathname first (sales-returns routes), then fall back to query param
  ...
  const [parties, setParties] = useState([]); // suppliers, farmers, or customers
  ...
  const fetchParties = async () => {
    try {
      if (isPurchaseReturn) {
        ...
        // Sales return - fetch based on customerType
      ...
      notifications.show({
        title: 'Error',
        message: `Failed to fetch ${isPurchaseReturn ? 'suppliers' : customerType === 'Farmer' ? 'farmers' : 'customers'}`,
        color: 'red'
      });
      setParties([]);
    }
  ...
  const fetchReturn = async (returnId) => {
    try {
      setLoading(true);
      const api = isPurchaseReturn ? dairyPurchaseReturnAPI : dairySalesReturnAPI;
      const response = await api.getById(returnId);
      const returnData = response?.data || response;

      if (!returnData) {
        notifications.show({ title: 'Error', message: 'Return not found', color: 'red' });
        return;
      }
      ...
      if (isPurchaseReturn) {
        ...
        setCustomerType(returnData.customerType || 'Farmer');
        form.setValues({
          returnDate: returnData.returnDate ? new Date(returnData.returnDate) : new Date(),
      ...
      notifications.show({ title: 'Error', message: 'Failed to fetch return', color: 'red' });
    } finally {
      setLoading(false);
    }
  };
    ...
    if (party) {
      ...
      if (isPurchaseReturn) {
        ...
        form.setValues({
          ...form.values,
          partyId: partyId,
          partyName: party.personalDetails?.name || '',
          partyPhone: party.personalDetails?.phone || '',
          partyAddress: party.personalDetails?.address || '',
          partyGstin: '',
          partyState: 'Tamil Nadu'
        });
      } else {
        form.setValues({
          ...
          partyPhone: party.phone || '',
          partyAddress: party.address || '',
          partyGstin: party.gstin || party.gstNumber || '',
          partyState: party.state || 'Tamil Nadu'
        });
      }
    }
  ...
  const handleAddItem = () => {
    ...
    if (isPurchaseReturn && qty > item.currentBalance) {
      notifications.show({
        ...
        message: `Available: ${item.currentBalance} ${item.unit || item.measurement}`,
        color: 'red'
      });
      return;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notifications.show({
        title: 'Error',
        message: error.message || `Failed to save ${isPurchaseReturn ? 'purchase' : 'sales'} return`,
        color: 'red'
      });
    } finally {
      setSaving(false);
  ...
  const resetForm = () => {
    ...
    setSelectedParty(null);
    setCalculations({
      totalQty: 0, grossAmount: 0, itemDiscount: 0, taxableAmount: 0,
  ...
  const itemOptions = items.map(item => ({
    value: item._id,
    label: `${item.itemCode || ''} - ${item.itemName} (Stock: ${item.currentBalance} ${item.unit || item.measurement || ''})`
  }));
  ...
  const partyOptions = parties.map(party => {
    if (isPurchaseReturn) {
      ...
      return {
        value: party._id,
        label: `${party.farmerNumber || ''} - ${party.personalDetails?.name || ''} ${party.personalDetails?.phone ? `| ${party.personalDetails.phone}` : ''}`
      };
    } else {
      return {
        value: party._id,
        label: `${party.customerId || ''} - ${party.name || ''} ${party.phone ? `| ${party.phone}` : ''}`
      };
    }
  });
  ...
  const partyLabel = isPurchaseReturn ? 'Supplier' : (customerType === 'Farmer' ? 'Farmer' : 'Customer');
          ...
          <Group>
            ...
            <div>
              ...
              <Text size="sm" c="dimmed">
                {isPurchaseReturn
                  ? 'Debit Note - Return goods to supplier (Dairy)'
                  : 'Credit Note - Customer returns goods to you (Dairy)'}
              </Text>
            </div>
          </Group>
          <Group>
        ...
        <Grid.Col span={8}>
          <Stack gap="md">
            ...
            <Paper withBorder p="md" radius="md">
              ...
              <Grid gutter="sm">
                {(isPurchaseReturn || customerType !== 'Other') && (
                  <Grid.Col span={6}>
                    <Select
                ...
                <Grid.Col span={isPurchaseReturn || customerType !== 'Other' ? 6 : 12}>
                  <TextInput
                    ...
                    <div>
                      <Text size="sm" fw={600}>
                        {isPurchaseReturn
                          ...
                          : customerType === 'Farmer'
                            ? selectedParty.personalDetails?.name
                            : (selectedParty.name)}
                      </Text>
                      <Text size="xs" c="dimmed">
                        {isPurchaseReturn
                          ? selectedParty.phone
                          : customerType === 'Farmer'
                            ? selectedParty.personalDetails?.phone
                            : selectedParty.phone}
                      </Text>
                    </div>
      ...
      <div style={{ display: 'none' }}>
        <div ref={printRef} style={{ padding: '20px', fontFamily: 'Arial, sans-serif', fontSize: '12px' }}>
          <div style={{ textAlign: 'center', borderBottom: '2px solid #333', paddingBottom: '10px', marginBottom: '15px' }}>
            <h1 style={{ margin: '0', fontSize: '24px' }}>{selectedCompany?.companyName || 'DAIRY COOPERATIVE'}</h1>
                ...
                <tr>
                  ...
                  <td style={{ width: '50%', textAlign: 'right' }}>
                    <div style={{ marginTop: '40px' }}>
                      <strong>For {selectedCompany?.companyName || 'DAIRY COOPERATIVE'}</strong><br /><br /><br />
...
export default DairyReturnForm;
