/**
 * Business Income Voucher - For Private Firm
 * Exactly matches Dairy Receipt Voucher functionality
 */
...
const BusinessIncomeVoucher = () => {
  ...
  const fetchVouchers = async () => {
    ...
    try {
      const response = await businessVoucherAPI.getAll({ voucherType: 'Income' });
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      await businessVoucherAPI.createIncome(payload);
      notifications.show({
        title: 'Success',
      ...
      notifications.show({
        title: 'Error',
        message: error.message || 'Failed to create income voucher',
        color: 'red'
      });
    } finally {
      setLoading(false);
  ...
  const handlePrint = (voucher) => {
    ...
    setTimeout(() => {
      ...
      const creditEntry = voucher.entries?.find(e => e.type === 'credit' || e.creditAmount > 0);
      printWindow.document.write(`
        <!DOCTYPE html>
        <html>
        <head>
          <title>Income Voucher - ${voucher.voucherNumber || 'N/A'}</title>
          <style>
        ...
        <body>
          <div class="voucher-container">
            ...
            <div class="info-row bordered">
              ...
              <span class="value">${creditEntry?.ledgerName || creditEntry?.ledgerId?.name || '-'}</span>
            </div>
  ...
  const handleSaveMultiple = async () => {
    ...
    if (successCount > 0) {
      notifications.show({
        title: 'Success',
        message: `${successCount} income voucher(s) created successfully`,
        color: 'green'
      });
      setModalOpened(false);
      fetchVouchers();
  ...
  const getLedgerGroup = (group) => {
    ...
    if (['Direct Incomes', 'Indirect Incomes', 'Sales Accounts'].includes(group)) return 'Income Accounts';
    if (['Current Assets', 'Fixed Assets'].includes(group)) return 'Asset Accounts';
    ...
    <Container size="xl" py="md">
        ...
        <Group justify="space-between" align="center">
          <Group>
            <ThemeIcon size={40} radius="md" variant="light" color="green">
              <IconArrowDownCircle size={24} />
            </ThemeIcon>
            <div>
              <Title order={3}>Income Voucher</Title>
              <Text size="sm" c="dimmed">Manage income vouchers for money received</Text>
            </div>
          </Group>
          <Button
        ...
        {vouchers.length === 0 ? (
          <Card p="xl" withBorder bg="gray.0">
            <Stack align="center" gap="md">
              <IconArrowDownCircle size={48} color="gray" />
              <Text c="dimmed" size="lg">No income vouchers found</Text>
              <Button
                variant="light"
                leftSection={<IconPlus size={16} />}
                onClick={openAddModal}
              >
                Create First Income Voucher
              </Button>
            </Stack>
          </Card>
        ) : (
                ...
                {vouchers.map((voucher, index) => {
                  ...
                  return (
                    <Table.Tr key={voucher._id}>
                      ...
                      <Table.Td>{creditEntry?.ledgerName || creditEntry?.ledgerId?.name || '-'}</Table.Td>
      ...
      <Modal
        ...
        title={
          <Group gap="xs">
            <IconArrowDownCircle size={20} />
            <Text fw={600}>Add Income Voucher</Text>
          </Group>
        }
          ...
          {entryMode === 'single' ? (
            ...
            <Stack gap="md">
              <Alert color="green" variant="light" py="xs">
                <Text size="xs">
                  Add multiple income vouchers at once. Cash account will be debited automatically for each entry.
                </Text>
              </Alert>
      ...
      <Modal
        ...
        title={
          <Group gap="xs">
            <IconEye size={20} />
            <Text fw={600}>Income Voucher Details</Text>
          </Group>
        }
        size="md"
        centered
        ...
        {selectedVoucher && (
          <Stack gap="md">
            <Grid>
              ...
              <Grid.Col span={12}>
                <Text size="sm" c="dimmed">Head of Account (Received From)</Text>
                <Text fw={500}>
                  {selectedVoucher.entries?.find(e => e.type === 'credit' || e.creditAmount > 0)?.ledgerName ||
                   selectedVoucher.entries?.find(e => e.type === 'credit' || e.creditAmount > 0)?.ledgerId?.name || '-'}
                </Text>
              </Grid.Col>
              <Grid.Col span={12}>
      ...
      <div id="print-income" style={{ display: 'none' }} ref={printRef}></div>
    </Container>
  );
};

export default BusinessIncomeVoucher;
