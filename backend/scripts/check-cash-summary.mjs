// Verify Cash Advance summary opening/advanced/recovery/balance month by month.
...
const baseOpening = (await PO.findOne({ companyId: cObjId, farmerId: fObjId }))?.cashAdvance ?? 0;

const cashTypes = ['Cash Advance', 'Cash Recovery'];
async function recovery(filterDate) {
  const agg = await FP.aggregate([
    { $match: { companyId: cObjId, farmerId: fObjId, status: { $ne: 'Cancelled' }, 'deductions.type': { $in: cashTypes }, ...filterDate } },
    { $unwind: '$deductions' },
    { $match: { 'deductions.type': { $in: cashTypes } } },
...
async function advanced(filterDate) {
  const agg = await AD.aggregate([
    { $match: { companyId: cObjId, farmerId: fObjId, advanceCategory: 'Cash Advance', status: { $ne: 'Cancelled' }, ...filterDate } },
...
console.log(`Base opening (ProducerOpening.cashAdvance): ${baseOpening}\n`);
